...
* 특정 마이그레이션 파일이 데이터베이스에 반영될때 실행되는 쿼리문은 다음과 같이 확인할 수 있다.
...
> Django ORM을 활용하게 되면, 파이썬 객체조작으로 데이터베이스 조작이 가능하다.
>
> ORM (Object-Relational-Mapping)에서는 주로 활용되는 퀴리문들이 모두 method로 구성 되어 있다.
...
* 특정(단일) 데이터 조회
  ...
  * `.get()` 은 그 결과가 여러개 이거나 없는 경우 오류가 발생한다.

  * 따라서, 단일 데이터 조회시(`Primary Key` 를 통해) 에만 사용한다.
